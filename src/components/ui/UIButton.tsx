...
export const UIButton = ({ children, btnType="primary", onClick,isDisabled,className="" }: Props) => {
  const generateButton = () => {
    ...
    const buttons = {
        ...
        "border":  <button disabled={isDisabled} onClick={onClick} className={`cursor-pointer px-4 py-3 rounded-lg bg-neutral-0 dark:bg-neutral-100 text-neutral-950  dark:text-neutral-600 border border-neutral-300 dark:border-none disabled:opacity-55 disabled:cursor-not-allowed focus:shadow shadow-neutral-200 transition-all ease-in-out duration-300 ${className}`} > {children} </button>
    }
